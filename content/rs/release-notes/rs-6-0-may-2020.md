...
For more information, check out the [Redis Enterprise Software user management documentation]({{< relref "/rs/security/database-security/authentication-and-authorization.md" >}}).
...
To use the updated modules with a database, you must [upgrade the module on the database]({{< relref "/modules/add-module-to-cluster.md#upgrading-the-module-for-the-database" >}}).
...
- Added REST API and rladmin commands to modify the timeout for [automatically disconnecting an inactive web UI session]({{< relref "/rs/administering/access-control/_index.md#session-timeout" >}}).
